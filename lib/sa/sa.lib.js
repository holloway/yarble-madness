(function(){
	...
	window.sa = {
		...
		submitthread: function(forum_id, title, text_content, post_icon_id, callback){
			if(!cache.formkey || !cache.formcookie) return alert("Unable to submit post. Was never able to scrape a form.");
			var formkey     = cache.formkey,
				formcookie = cache.formcookie;
			
			if(post_icon_id === undefined) post_icon_id = 0;
			return request.post(
				http_base + 'newthread.php',
				undefined,
				{
				...
				response_filters.submitthread(callback, forum_id)
			);
		},
	...
	var request = {
		...
		failure: function(event){
			if(!request.failure_callbacks || request.failure_callbacks.length === 0){
				alert("Network error. If you're testing in desktop Chrome be sure to either use the Allow-Control-Allow-Origin: * extension, or disable CORS via command line");
			} else {
	...
	var response_filters = {
		...
		lastpost: function(fn, thread_id){
			return function(){
				...
				return fn.apply(this, [response.forum_id, response.thread_id, response.last_page_number, response.last_post_id]);
			};
		},
		...
		submitthread: function(fn, forum_id){
			return function(){
				console.log(this);
	...
	window.sa.response_parse = {
		...
		forum: function(html_string){
			...
			$subforums = $("#subforums", $div)[0];
			if($subforums){
				subforums = [];
				$subforums = $("tbody tr", $subforums);
				for(i = 0; i < $subforums.length; i++){
					subforum_item = {};
					$subforum = $subforums[i];
					$subforum_link = $("dl a", $subforum)[0];
					subforum_item.forum_id = get_param($subforum_link.getAttribute("href"), "forumid");
					subforum_item.title = $subforum_link.innerText;
					subforum_item.subtitle = $("dd", $subforum)[0].innerText;
					subforums.push(subforum_item);
				}
				response.subforums = subforums;
			}
			return response;
		},
		...
		lastpost: function(html_string){
			...
			$tables = $("table", $div);
			if($tables.length) {
				response.last_post_id = $tables[$tables.length - 1].getAttribute("id").replace(/^post/, '');
			}
			return response;
		}
	};
