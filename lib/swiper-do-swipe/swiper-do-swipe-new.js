(function($){
        ...
        _this = {
                ...
                animate: function(){ //called under a requestAnimationFrame at ~60fps, initially from pointer.start
                    ...
                    switch(pointer.drag_direction){
                        ...
                        case "vertical":
                            ...
                            event.preventDefault();
                            event.stopPropagation();
                            console.log("vertical", $page_current.scroll_y);
                            break;
                        default: // then the touch/click has ended without a horizontal/vertical scroll, so it's a 'click'
                            $page_current = _this.$pages[index];
                            console.log("click", $page_current.scroll_y);
                            // nothing
                    }
                    pointer.drag_direction = false;
            ...
            touch: {
                start: function(event){
                    ...
                    if(is_form_widget(target)) return;
            ...
            mouse: {
                ...
                start: function(event){
                    ...
                    if(event.button === 2) return; // if(right-click) return
                    if(!buggy_effects_on_android) {
                        event.preventDefault();
                        //event.stopPropagation();
                    }
            ...
            scroll_to_y: function(pixels){
                var $page = _this.$pages[_this.index];

                $page.scroll_y = pixels;
                if($page.scroll_y < 0) {
                     $page.scroll_y = 0;
            ...
            mousewheel: function(event){
                ...
                _this.latest_inertia_scroll_movements = [0, -event.wheelDeltaY / 20]; // 20 is an arbitrary 'magic number' that feels like a normal mouse wheel scroll (on Ubuntu 13.10, in Chromium)
                _this.inertia_scroll_start();
            },
            ...
            inertia_scroll: function(){
                ...
                if(Math.abs(_this.inertia_remaining) > _this.options.stop_inertia_scrolling_at){
                    ...
                    console.log("after inertia", $page.scroll_y)
    ...
    window.reduce_size_before_horizontal_scroll = function($pages, index, reduced_by_amount){
        ...
        $page_current.style.overflow = "hidden";
        ...
        if($page_before){
            ...
            $page_before.style.overflow = "hidden";
        ...
        if($page_after){
            ...
            $page_after.style.overflow = "hidden";
